...
POPULATION_SIZE = 25 #LICZBA POPULACJI
CHROMOSOME_LENGTH = 2  
X_BOUND = [-512, 512]  
CROSSOVER_RATE = 0.75 #Pc
MUTATION_RATE = 0.05 #Pm
ELITISM_COUNT = 2 #Liczba elit
...
def fitness(individual):
    return eggholder_function(individual)  # Wartość do minimalizacji
...
def mutate(individual, generation, max_generations):
    # Możesz zaimplementować adaptacyjne prawdopodobieństwo mutacji, jeśli chcesz
...
def genetic_algorithm():
    ...
    while not global_min_found and generation < max_generations:
    ...
    plt.colorbar(contour, label='Głębokość poziomic')  # Dodanie skali głębokości
    ...
    run_number = 1
    if os.path.isfile('wyniki_eggholder.csv'):
        with open('wyniki_eggholder.csv', mode='r', newline='', encoding='utf-8') as csv_file:
    ...
    file_exists = os.path.isfile('wyniki_eggholder.csv')
    with open('wyniki_eggholder.csv', mode='a', newline='', encoding='utf-8') as csv_file:
        fieldnames = [
            'Numer uruchomienia',
    ...
    print("\nWyniki zostały zapisane do pliku 'wyniki_eggholder.csv'.")
