...
def gField(PopSize, dim, pos, M, l, iters, G, ElitistCheck, Rpower, e_constant):
    if ElitistCheck == 1:
        ...
        kbest = max(kbest, 1)  # Upewniamy się, że kbest jest co najmniej 1
    else:
        kbest = PopSize
...
def GSA(objf, lb, ub, dim, PopSize, iters, G0=2.0, alpha=20.0, Rpower=2.0, e_constant=0.01, GraviPert_Min=0.2, GraviPert_Max=0.6, VelocityPert_Min=0.0, VelocityPert_Max=1.0, tol=1e-3):
    ...
    # Wykres konwergencji
    plot_convergence(convergence_curve, avg_fitness_curve)

    # Wykres różnorodności
    plot_diversity(diversity_curve)
...
def save_results_to_csv(s, PopSize):
    run_number = 1
    filename = 'wyniki_gsa_michalewicz.csv'
    ...
    results = {
        ...
        'Po ilu iteracjach znaleziono rozwiązanie': s.iterations,
    ...
    print("\nWyniki zostały zapisane do pliku 'wyniki_gsa_michalewicz.csv'.")
...
# Definicja funkcji Michalewicza
def michalewicz(x, m=10):
    x = numpy.asarray(x)
    i = numpy.arange(1, len(x)+1)
    return -numpy.sum(numpy.sin(x) * (numpy.sin(i * x**2 / numpy.pi)) ** (2 * m))

# Znane globalne minimum dla funkcji Michalewicza w wymiarze 5
...
if __name__ == "__main__":
    ...
    PopSize = 25
    iters = 1000
    G0 = 150.0          # Początkowa stała grawitacji
    alpha = 20.0      # Parametr alpha kontrolujący spadek G
    Rpower = 2.0
    e_constant = 0.01
    GraviPert_Min = 0.2
    GraviPert_Max = 0.6
