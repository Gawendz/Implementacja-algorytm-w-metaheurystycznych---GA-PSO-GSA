...
def save_results_to_csv(s, PopSize):
    run_number = 1
    filename = 'wyniki_gsa_eggholder.csv'
    ...
    print("\nWyniki zostały zapisane do pliku 'wyniki_gsa_eggholder.csv'.")
...
def eggholder(x):
    x1 = x[0]
    x2 = x[1]
    term1 = -(x2 + 47) * numpy.sin(numpy.sqrt(abs(x2 + x1/2 + 47)))
    term2 = -x1 * numpy.sin(numpy.sqrt(abs(x1 - (x2 + 47))))
    return term1 + term2

# Znane globalne minimum dla funkcji Eggholder
...
if __name__ == "__main__":
    ...
    PopSize = 100
    iters = 1000
    G0 = 150          # Początkowa stała grawitacji
